...
///
std::string buildKernelNameFromRecordType(const clang::QualType &RecordType,
...
inline std::string buildKernelName(clang::RecordDecl *D, clang::MangleContext *Mangler) {
  ...
  assert(Mangler);
  auto DeclName =
      buildKernelNameFromRecordType(Mangler->getASTContext().getTypeDeclType(D), Mangler);
  return "__acpp_kernel_" + DeclName;
}

// Partially taken from CGCUDANV.cpp
inline std::string getDeviceSideName(clang::NamedDecl *ND, clang::ASTContext &Ctx,
...
FrontendASTVisitor::FrontendASTVisitor(clang::CompilerInstance &instance) : Instance{instance} {
  ...
  if (instance.getLangOpts().CUDAIsDevice)
    ...
    NameMangler = clang::ItaniumMangleContext::create(instance.getASTContext(),
...
FrontendASTVisitor::~FrontendASTVisitor() {}

bool FrontendASTVisitor::shouldVisitTemplateInstantiations() const { return true; }
bool FrontendASTVisitor::shouldVisitImplicitCode() const { return true; }

bool FrontendASTVisitor::VisitStmt(clang::Stmt *S) {
...
bool FrontendASTVisitor::VisitDecl(clang::Decl *D) {
...
bool FrontendASTVisitor::VisitFunctionDecl(clang::FunctionDecl *f) {
  if (!f)
    return true;
...
bool FrontendASTVisitor::VisitCallExpr(clang::CallExpr *Call) {
...
void FrontendASTVisitor::applyAttributes() {
  ...
  for (auto F : MarkedKernels) {
    if (!F->hasAttr<clang::CUDAGlobalAttr>() && CustomAttributes::SyclKernel.isAttachedTo(F)) {
...
std::unordered_set<clang::FunctionDecl *> &FrontendASTVisitor::getMarkedHostDeviceFunctions() {
  return MarkedHostDeviceFunctions;
}
void FrontendASTVisitor::markAsHostDevice(clang::FunctionDecl *F) {
  this->MarkedHostDeviceFunctions.insert(F);
}
void FrontendASTVisitor::markAsKernel(clang::FunctionDecl *F) { this->MarkedKernels.insert(F); }
void FrontendASTVisitor::markAsNDKernel(clang::FunctionDecl *F) { this->HostNDKernels.insert(F); }
void FrontendASTVisitor::markAsSSCPOutliningEntrypoint(clang::FunctionDecl *F) {
  this->SSCPOutliningEntrypoints.insert(F);
}
void FrontendASTVisitor::processFunctionDecl(clang::FunctionDecl *f) {
  if (!f)
    return;

  if (f->getQualifiedNameAsString() == "hipsycl::glue::hiplike_dispatch::parallel_for_workgroup") {
...
bool FrontendASTVisitor::isPrivateMemory(const clang::VarDecl *V) const {
...
bool FrontendASTVisitor::isLocalMemory(const clang::VarDecl *V) const {
  const clang::CXXRecordDecl *R = V->getType()->getAsCXXRecordDecl();
  if (R)
...
void FrontendASTVisitor::storeLocalVariablesInLocalMemory(clang::Stmt *BlockStmt,
...
void FrontendASTVisitor::storeVariableInLocalMemory(clang::VarDecl *V) const {
...
const clang::RecordType *FrontendASTVisitor::getTemplateTypeArgument(clang::FunctionDecl *F,
                                                                     int TemplateArg) {
  ...
  if (Info) {
    ...
    if (KernelNameArgument.getKind() == clang::TemplateArgument::ArgKind::Type) {
      if (auto RecordType =
              llvm::dyn_cast<clang::RecordType>(KernelNameArgument.getAsType().getTypePtr())) {
        return RecordType;
      }
    }
  }
...
const clang::RecordType *FrontendASTVisitor::getKernelNameTag(clang::FunctionDecl *F) {
  return getTemplateTypeArgument(F, 0);
}
bool FrontendASTVisitor::isKernelUnnamed(clang::FunctionDecl *F) {
  if (!F)
    return false;
  ...
  if (NameTag->getDecl()) {
    return NameTag->getDecl()->getQualifiedNameAsString() == "__acpp_unnamed_kernel";
  }

  return true;
}
const clang::RecordType *
FrontendASTVisitor::getRelevantKernelNamingComponent(clang::FunctionDecl *F) {
  if (isKernelUnnamed(F)) {
    auto BodyIterator = KernelBodies.find(F);

    if (BodyIterator == KernelBodies.end()) {
      HIPSYCL_DEBUG_ERROR << "Kernel did not have body registered, this should never happen\n";
      return nullptr;
    }
...
bool FrontendASTVisitor::handleKernelStub(clang::FunctionDecl *F) {
...
bool FrontendASTVisitor::handleKernel(clang::FunctionDecl *F, const clang::RecordType *KernelBody) {
  UserKernels.insert(F);
  KernelBodies[F] = KernelBody;
  return true;
}
void FrontendASTVisitor::setKernelName(clang::FunctionDecl *F, const std::string &name) {
  ...
  if (name.empty()) {
    ...
    auto ID = Instance.getASTContext().getDiagnostics().getCustomDiagID(
        clang::DiagnosticsEngine::Level::Error, "No valid kernel name for kernel submission");
  ...
  if (!F->hasAttr<clang::AsmLabelAttr>()) {
    F->addAttr(clang::AsmLabelAttr::CreateImplicit(Instance.getASTContext(), name));
    HIPSYCL_DEBUG_INFO << "AST processing: Adding ASM label attribute with kernel name " << name
                       << "\n";
  }
}

void FrontendASTVisitor::nameKernelUsingTypes(clang::FunctionDecl *F, bool RenameUnnamedKernels) {
  std::string KernelName;

  // If we are dealing with a named kernel, construct the name
  // based on the kernel name argument
  if (!isKernelUnnamed(F)) {
    KernelName = buildKernelName(getKernelNameTag(F)->getAsRecordDecl(), KernelNameMangler.get());
    ...
    if (RenameUnnamedKernels) {
      ...
      if (KernelFunctorArgument) {
        KernelName =
            buildKernelName(KernelFunctorArgument->getAsRecordDecl(), KernelNameMangler.get());
        setKernelName(F, KernelName);
      }
    }
  }
}
void FrontendASTVisitor::nameKernelUsingUniqueMangler(clang::FunctionDecl *F) {
...
void FrontendASTVisitor::nameKernelUsingKernelManglingStub(clang::FunctionDecl *F) {
  ...
  std::string KernelName =
      getDeviceSideName(SuggestionIt->second, Instance.getASTContext(), KernelNameMangler.get(),
...
void FrontendASTVisitor::nameKernel(clang::FunctionDecl *F) {
...
FrontendASTConsumer::FrontendASTConsumer(clang::CompilerInstance &I) : Visitor{I}, Instance{I} {
  CompilationStateManager::get().reset();
}

bool FrontendASTConsumer::HandleTopLevelDecl(clang::DeclGroupRef DG) {
  for (auto &&D : DG)
    Visitor.TraverseDecl(D);
  return true;
}

void FrontendASTConsumer::HandleTranslationUnit(clang::ASTContext &context) {
